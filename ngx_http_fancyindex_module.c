...
#define DATETIME_FORMATS(F_, t)                                                \
  ...
  F_('F', 10, "%d-%02d-%02d", (t)->ngx_tm_year, (t)->ngx_tm_mon,               \
     (t)->ngx_tm_mday)                                                         \
  ...
  F_('P', 2, "%2s", (((t)->ngx_tm_hour < 12) ? "am" : "pm"))                   \
  F_('r', 11, "%02d:%02d:%02d %2s", ((t)->ngx_tm_hour % 12) + 1,               \
     (t)->ngx_tm_min, (t)->ngx_tm_sec,                                         \
     (((t)->ngx_tm_hour < 12) ? "AM" : "PM"))                                  \
  F_('R', 5, "%02d:%02d", (t)->ngx_tm_hour, (t)->ngx_tm_min)                   \
  F_('S', 2, "%02d", (t)->ngx_tm_sec)                                          \
  F_('T', 8, "%02d:%02d:%02d", (t)->ngx_tm_hour, (t)->ngx_tm_min,              \
     (t)->ngx_tm_sec)                                                          \
...
static size_t ngx_fancyindex_timefmt_calc_size(const ngx_str_t *fmt) {
...
static u_char *ngx_fancyindex_timefmt(u_char *buffer, const ngx_str_t *fmt,
                                      const ngx_tm_t *tm) {
#define DATETIME_CASE(letter, fmtlen, fmt, ...)                                \
  case letter:                                                                 \
    buffer = ngx_snprintf(buffer, fmtlen, fmt, ##__VA_ARGS__);                 \
    break;
...
typedef struct {
  ...
  ngx_flag_t show_path; /**< Whether to display or not the path + '</h1>' after
                           the header */
...
static ngx_uint_t headerfooter_kind(const ngx_str_t *value) {
  static const struct {
    ngx_str_t name;
    ngx_uint_t value;
  } values[] = {
...
static char *ngx_fancyindex_conf_set_headerfooter(ngx_conf_t *cf,
                                                  ngx_command_t *cmd,
                                                  void *conf) {
  ...
  if (kind == NGX_HTTP_FANCYINDEX_HEADERFOOTER_LOCAL) {
    ...
    while (n > 0) {
      ssize_t r =
          ngx_read_file(&file, item->local.data + file.offset, n, file.offset);
      if (r == NGX_ERROR) {
        ngx_close_file(file.fd);
        ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
 ...
 */
#define ngx_cpymem_ssz(_p, _t) (ngx_cpymem((_p), (_t), sizeof(_t) - 1))

/**
 * Copy a ngx_str_t.
 */
#define ngx_cpymem_str(_p, _s) (ngx_cpymem((_p), (_s).data, (_s).len))

/**
 * Check whether a particular bit is set in a particular value.
 */
#define ngx_has_flag(_where, _what) (((_where) & (_what)) == (_what))
...
static char *ngx_http_fancyindex_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                                void *child);

static char *ngx_http_fancyindex_ignore(ngx_conf_t *cf, ngx_command_t *cmd,
                                        void *conf);

static uintptr_t ngx_fancyindex_escape_filename(u_char *dst, u_char *src,
                                                size_t size);
...
make_header_buf(ngx_http_request_t *r,
                const ngx_str_t css_href) ngx_force_inline;
...
static ngx_command_t ngx_http_fancyindex_commands[] = {
    ...
    {ngx_string("fancyindex"),
     ...
     offsetof(ngx_http_fancyindex_loc_conf_t, enable), NULL},
    ...
    {ngx_string("fancyindex_case_sensitive"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, case_sensitive), NULL},

    {ngx_string("fancyindex_directories_first"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, dirs_first), NULL},

    {ngx_string("fancyindex_localtime"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, localtime), NULL},

    {ngx_string("fancyindex_exact_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, exact_size), NULL},

    {ngx_string("fancyindex_header"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE12,
     ngx_fancyindex_conf_set_headerfooter, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, header), NULL},

    {ngx_string("fancyindex_footer"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE12,
     ngx_fancyindex_conf_set_headerfooter, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, footer), NULL},

    {ngx_string("fancyindex_css_href"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, css_href), NULL},

    {ngx_string("fancyindex_ignore"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_1MORE,
     ngx_http_fancyindex_ignore, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("fancyindex_hide_symlinks"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, hide_symlinks), NULL},

    {ngx_string("fancyindex_show_path"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, show_path), NULL},

    {ngx_string("fancyindex_show_dotfiles"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, show_dot_files), NULL},

    {ngx_string("fancyindex_hide_parent_dir"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, hide_parent), NULL},

    {ngx_string("fancyindex_time_format"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_fancyindex_loc_conf_t, time_format), NULL},
...
static const ngx_str_t css_href_post = ngx_string("\" type=\"text/css\"/>\n");

#ifdef NGX_ESCAPE_URI_COMPONENT
static inline uintptr_t ngx_fancyindex_escape_filename(u_char *dst, u_char *src,
                                                       size_t size) {
...
static ngx_inline ngx_buf_t *make_header_buf(ngx_http_request_t *r,
                                             const ngx_str_t css_href) {
  ngx_buf_t *b;
  size_t blen = r->uri.len + ngx_sizeof_ssz(t01_head1) +
                ngx_sizeof_ssz(t02_head2) + ngx_sizeof_ssz(t03_head3) +
                ngx_sizeof_ssz(t04_body1);

  if (css_href.len) {
    blen += css_href_pre.len + css_href.len + css_href_post.len;
  }
...
make_content_buf(ngx_http_request_t *r, ngx_buf_t **pb,
  ...
  static const int64_t exbibyte =
      1024LL * 1024LL * 1024LL * 1024LL * 1024LL * 1024LL;

  /*
   * NGX_DIR_MASK_LEN is lesser than NGX_HTTP_FANCYINDEX_PREALLOCATE
   */
  if ((last = ngx_http_map_uri_to_path(
           r, &path, &root, NGX_HTTP_FANCYINDEX_PREALLOCATE)) == NULL)
  ...
  for (;;) {
...
#if NGX_PCRE
    {
      ...
      if (alcf->ignore &&
          ngx_regex_exec_array(alcf->ignore, &str, r->connection->log) !=
              NGX_DECLINED) {
        continue;
      }
    }
    ...
    if (!dir.valid_info) {
      ...
      if (path.len + 1 + len + 1 > allocated) {
        allocated = path.len + 1 + len + 1 + NGX_HTTP_FANCYINDEX_PREALLOCATE;
    ...
    entry->escape =
        2 * ngx_fancyindex_escape_filename(NULL, ngx_de_name(&dir), len);
    entry->escape_html =
    ...
    entry->utf_len = (r->headers_out.charset.len == 5 &&
                      ngx_strncasecmp(r->headers_out.charset.data,
                                      (u_char *)"utf-8", 5) == 0)
  ...
  if (alcf->show_path)
    len = r->uri.len + escape_html + ngx_sizeof_ssz(t05_body2) +
          ngx_sizeof_ssz(t06_list1) + ngx_sizeof_ssz(t_parentdir_entry) +
          ngx_sizeof_ssz(t07_list2) +
          ngx_fancyindex_timefmt_calc_size(&alcf->time_format) * entries.nelts;
  else
    len = r->uri.len + escape_html + ngx_sizeof_ssz(t06_list1) +
          ngx_sizeof_ssz(t_parentdir_entry) + ngx_sizeof_ssz(t07_list2) +
          ngx_fancyindex_timefmt_calc_size(&alcf->time_format) * entries.nelts;
  ...
  for (i = 0; i < entries.nelts; i++) {
    ...
    len += ngx_sizeof_ssz("<tr><td colspan=\"2\" class=\"link\"><a href=\"") +
           ...
           + ngx_sizeof_ssz("\" title=\"") + entry[i].name.len +
           entry[i].utf_len + entry[i].escape_html + ngx_sizeof_ssz("\">") +
           entry[i].name.len + entry[i].utf_len + entry[i].escape_html +
           ngx_sizeof_ssz("</a></td><td class=\"size\">") + 20 /* File size */
  ...
  if ((r->args.len == 3 || (r->args.len == 7 && r->args.data[3] == '&')) &&
    ...
    ngx_int_t sort_descending = r->args.len == 7 && r->args.data[4] == 'O' &&
                                r->args.data[5] == '=' &&
                                r->args.data[6] == 'D';
  ...
  if (alcf->show_path) {
    b->last = last =
        (u_char *)ngx_escape_html(b->last, r->uri.data, r->uri.len);
  ...
  if (r->uri.len > 1 && alcf->hide_parent == 0) {
    ...
    if (*sort_url_args) {
      b->last =
          ngx_cpymem(b->last, sort_url_args, ngx_sizeof_ssz("?C=N&amp;O=A"));
    }
    b->last = ngx_cpymem_ssz(b->last, "\">Parent directory/</a></td>"
                                      "<td class=\"size\">-</td>"
                                      "<td class=\"date\">-</td>"
                                      "</tr>" CRLF);
  }
  ...
  for (i = 0; i < entries.nelts; i++) {
    b->last = ngx_cpymem_ssz(b->last,
                             "<tr><td colspan=\"2\" class=\"link\"><a href=\"");

    if (entry[i].escape) {
      ngx_fancyindex_escape_filename(b->last, entry[i].name.data,
                                     entry[i].name.len);
    ...
    if (entry[i].dir) {
      *b->last++ = '/';
      if (*sort_url_args) {
        b->last =
            ngx_cpymem(b->last, sort_url_args, ngx_sizeof_ssz("?C=x&amp;O=y"));
      }
    }
      ...
      if (entry[i].dir) {
        *b->last++ = '-';
        ...
        if (j == DIM(sizes) - 1)
          ...
          b->last = ngx_sprintf(b->last, "%.1f %s", (float)length / multiplier,
                                sizes[j]);
      }
    }
...
static ngx_int_t ngx_http_fancyindex_handler(ngx_http_request_t *r) {
  ngx_http_request_t *sr;
  ngx_str_t *sr_uri;
  ngx_str_t rel_uri;
  ngx_int_t rc;
  ngx_http_fancyindex_loc_conf_t *alcf;
  ngx_chain_t out[3] = {{NULL, NULL}, {NULL, NULL}, {NULL, NULL}};
  ...
  if (alcf->header.path.len > 0 && alcf->header.local.len == 0) {
    ...
    if (rc == NGX_ERROR || rc == NGX_DONE) {
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                     "http fancyindex: header subrequest for \"%V\" failed",
                     sr_uri);
      return rc;
    }
  ...
  if (alcf->footer.path.len == 0 || alcf->footer.local.len > 0) {
    ngx_uint_t last =
        ((alcf->header.path.len > 0) && (alcf->header.local.len == 0)) ? 1 : 2;
  ...
  if (rc == NGX_ERROR || rc == NGX_DONE) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http fancyindex: footer subrequest for \"%V\" failed",
                   sr_uri);
    return rc;
  }
...
static char *ngx_http_fancyindex_merge_loc_conf(ngx_conf_t *cf, void *parent,
  ...
  ngx_conf_merge_str_value(conf->time_format, prev->time_format,
                           "%Y-%b-%d %H:%M");
  ...
  if (conf->show_path == 0 && conf->header.path.len == 0) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "FancyIndex : cannot set show_path to off without "
                       "providing a custom header !");
    return NGX_CONF_ERROR;
  }
